...
pub struct SteamGame {
    ...
    /// Unix seconds of the ACF's `LastUpdated` (last time Steam touched the
    /// install). None when ACF reports 0.
    pub last_updated_ts: Option<u64>,
    /// `BytesToDownload` — bytes Steam still needs to fetch for this app.
    pub bytes_to_download: u64,
    /// `BytesDownloaded` — bytes fetched so far for the in-progress update
    /// (0 when fully installed).
    pub bytes_downloaded: u64,
...
///
/// System-level components (SteamVR runtime, Steamworks redistributables,
/// Steam client itself) are never recommended — uninstalling them breaks
/// other games, so the UI must not surface them as reclaimable.
/// appid → 名称（仅注释，不参与逻辑）：
///   30        Steam（客户端本体）
///   240       Steam App（客户端辅助）
///   480       Steam Client（旧版客户端）
///   675        Steamworks Common Redistributables
///   713       Steam Client Bootstrapper
///   570       Steamworks Shared Redistributables
///   105600    Steam Remote Play
///   228980    SteamVR for HTC Vive
///   250820    SteamVR（运行时）
///   446873    SteamVR for Valve Index
///   1675200   Steam Client Beta
const SYSTEM_APPS: &[u32] = &[
    30, 240, 480, 675, 570, 713, 105600, 228980, 250820, 446873, 1675200,
];
...
fn read_library(lib_root: &Path, now_secs: u64) -> Result<SteamLibrary> {
        ...
        .with_context(|| format!("read_dir {}", steamapps.display()))?
    {
        ...
        // ACF 的 installdir 为空（Steam 允许）时 `join("")` 会解析成
        // `steamapps/common` 本身——这个目录通常存在，导致 ghost 误判为
        // 正常安装。空 installdir 一律视为 ghost（没有任何安装目录）。
        let install_path = steamapps.join("common").join(&raw.install_dir);
        let is_ghost = raw.install_dir.is_empty() || !install_path.is_dir();
        let (default_recommended, recommendation_reason) = compute_recommendation(
            raw.size_on_disk,
            raw.last_played,
            is_ghost,
            now_secs,
        ...
        games.push(SteamGame {
            ...
            last_updated_ts: raw.last_updated,
            bytes_to_download: raw.bytes_to_download,
    ...
    // 汇总只统计真实安装（非 ghost）：「鬼魂安装」是 ACF 残留，其
    // SizeOnDisk 不代表实际占用，计入会让总览/列表总和虚高。未完成的
    // 更新也按 ghost 口径排除（bytes 还没真正落盘成可删占用）。
    let total_size_bytes = games
        .iter()
        .filter(|g| !g.is_ghost)
        .map(|g| g.size_bytes)
        .sum();
...
"#;
    ...
    fn ghost_always_recommended_with_diagnostic_reason() {
        let (rec, reason) =
            compute_recommendation(50_000_000_000, Some(TEST_NOW), true, TEST_NOW, 1001);
        assert!(rec);
    ...
    fn small_game_never_recommended() {
        ...
        let (rec, reason) = compute_recommendation(2_000_000_000, None, false, TEST_NOW, 1002);
        assert!(!rec);
    ...
    fn large_old_game_recommended_with_size_and_age_reason() {
        ...
        let (rec, reason) =
            compute_recommendation(60_000_000_000, Some(played_8mo_ago), false, TEST_NOW, 1003);
        assert!(rec);
        let r = reason.unwrap();
    ...
    fn never_played_large_game_recommended_with_explicit_never_string() {
        ...
        let (rec, reason) = compute_recommendation(60_000_000_000, None, false, TEST_NOW, 1004);
        assert!(rec);
    ...
    fn medium_recently_played_not_recommended() {
        ...
        let (rec, _) =
            compute_recommendation(40_000_000_000, Some(played_2mo_ago), false, TEST_NOW, 1005);
    ...
    fn very_large_recently_played_recommended() {
        ...
        let (rec, reason) =
            compute_recommendation(60_000_000_000, Some(played_3mo_ago), false, TEST_NOW, 1006);
        assert!(rec);
    ...
    fn very_old_game_says_n_years() {
        ...
        let (rec, reason) =
            compute_recommendation(70_000_000_000, Some(played_3y_ago), false, TEST_NOW, 1234);
        assert!(rec);
        assert!(reason.unwrap().contains("3 年未启动"));
    }

    #[test]
    fn system_component_never_recommended_even_if_ghost() {
        // SteamVR runtime (250820) is a system component: even a huge,
        // never-played, ghost ACF must never surface as "建议处理" — removing
        // it breaks every VR game.
        let (rec, reason) = compute_recommendation(80_000_000_000, None, true, TEST_NOW, 250820);
        assert!(!rec);
        assert!(reason.is_none());
    }

    #[test]
    fn steam_client_itself_never_recommended() {
        // Steam client (appid 480) also carries the "installdir = steam"
        // marker; the recommendation must not suggest uninstalling Steam.
        let (rec, _) = compute_recommendation(30_000_000_000, None, false, TEST_NOW, 480);
        assert!(!rec);
    }
