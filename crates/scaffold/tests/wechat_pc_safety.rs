...
fn load_wechat_pc() -> diskpilot_scaffold::Scaffold {
...
fn wechat_pc_matcher_distinguishes_data_from_installs() {
    ...
    let assert_match = |path: &Path, expected: Option<&str>, label: &str| {
        let got = diskpilot_scaffold::detect_for(&scaffolds, path);
        assert_eq!(
            got.as_deref(),
            expected,
    ...
    let compiled = diskpilot_scaffold::compile_all(&scaffolds);
    let assert_compiled = |path: &Path, expected: Option<&str>, label: &str| {
        let got = diskpilot_scaffold::detect_compiled(&compiled, path, None);
        assert_eq!(
            got.as_deref(),
            expected,
