# DiskPilot · 给 Claude 的工作约定

> 这份文件是仓库级硬约束，每次 Claude Code 会话会自动读。**改 scaffold / 加 scaffold 时 Claude 必须按本文件 + AGENTS.md 的 scaffold 流程走**。

## 项目背景

DiskPilot 是一个磁盘清理桌面 app（Tauri 2 + React + Rust）。核心是一组 **scaffold TOML**（`scaffolds/<id>.toml`）——每份描述一个常用软件的清理目标（缓存、日志、可选的历史媒体）和它的红线。Studio 面板按 scaffold 渲染卡片，用户可以按 scope 单独清理。
...
3. **UI 防御三件套**：任何调用破坏性 Tauri 命令（`recycle_paths` / `execute_scope`）的前端组件必须满足：
   ...
   - 后端同口径硬校验：真实执行（`dry_run=false`）必须带 `confirmed=true`，否则直接拒绝
     ——前端的确认窗只是体验层，绕过它 `invoke` 也删不掉任何东西
...
- 验证：`cargo check -p diskpilot-desktop` + `pnpm -C apps/desktop exec tsc --noEmit` 都干净

## 命令速查

```bash
# Scaffold lint（必须 0 error）
cargo run -p diskpilot-scaffold-lint -- scaffolds/<id>.toml
# 全部 safety test
...
- 完整 14-phase 工作流见 [development.md](development.md) 的 scaffold 章节
