...
const CACHE_KEY = 'diskpilot.workshopTitles';
...
export function SteamWorkshopModal({
  ...
  game: SteamGame;
  /** 可选中文名（来自 Inspector 的 nameCn 翻译表）；缺省回落英文名。 */
  nameCn?: string | null;
  onClose: () => void;
}) {
  const t = useT();
      ...
      <div className="steam-modal-dialog steam-workshop-dialog" role="dialog" aria-modal="true">
        <div className="steam-modal-head">
          <div className="steam-modal-title">
            <Boxes size={16} /> {t('steam.workshopTitle')} · {nameCn ?? game.name_en}
          </div>
          <div className="steam-modal-subtitle">
            {loading
              ? t('steam.workshopComputing')
              : t('steam.workshopTotal', { n: items?.length ?? 0, size: formatBytes(totalSize) })}
          </div>
          <button className="steam-modal-close" onClick={onClose} title={t('steam.closeEsc')} aria-label={t('steam.closeEsc')}>
            <X size={16} />
          </button>
        </div>
        ...
        <div className="steam-modal-body steam-workshop-body">
          ...
          {!loading && items && items.length > 0 && (
            <div className="steam-workshop-toolbar">
              <span className="steam-workshop-toolbar-label">{t('steam.workshopSort')}</span>
              <button
                className={'steam-workshop-sortbtn' + (sortBy === 'oldest' ? ' active' : '')}
                onClick={() => setSortBy('oldest')}
              >
                {t('steam.workshopSortOldest')}
              </button>
              <button
                className={'steam-workshop-sortbtn' + (sortBy === 'size' ? ' active' : '')}
                onClick={() => setSortBy('size')}
              >
                {t('steam.workshopSortSize')}
              </button>
            </div>
          )}
          ...
          {loading && (
            <div className="steam-workshop-loading">
              ...
              <span>{t('steam.workshopScanning', { n: game.workshop_item_count })}</span>
            </div>
          )}

          {error && <div className="steam-workshop-error">{t('steam.workshopScanFailed', { msg: error })}</div>}

          {!loading && !error && items && items.length === 0 && (
            <div className="steam-workshop-empty">
              {t('steam.workshopEmpty')}
            </div>
          )}
  ...
  return (
    <li className="steam-workshop-item">
      ...
      <button
        ...
        title={t('steam.workshopOpenTitle')}
      >
        <ExternalLink size={12} />
        <span>{t('steam.workshopOpen')}</span>
      </button>
    </li>
  );
}
...
function relativeTime(ts: number): string {
  if (ts === 0) return t('steam.wsUnknown');
  const now = Date.now() / 1000;
  const diff = now - ts;
  const day = 86400;
  const month = day * 30.4375;
  const year = month * 12;
  if (diff < 0) return t('steam.wsJustNow');
  if (diff < day) return t('steam.wsToday');
  if (diff < 7 * day) return t('steam.wsDaysAgo', { n: Math.floor(diff / day) });
  if (diff < month) return t('steam.wsWeeksAgo', { n: Math.floor(diff / day / 7) });
  if (diff < year) return t('steam.wsMonthsAgo', { n: Math.floor(diff / month) });
  return t('steam.wsYearsAgo', { n: Math.floor(diff / year) });
}
