...
export function ProgressButton({
  ...
  const t = useT();
  // 默认运行文案在渲染处取：参数默认值里拿不到 hook 返回的 t。
  const runningText = runningLabel ?? t('shell.progressbutton.cleaning');
  ...
  return (
    ...
    >
      ...
      <span className="progress-button-content">
        {state === 'revealing' ? (
          <>
            {runningText}… <span className="progress-button-pct">{pct}%</span>
          </>
        ) : state === 'error' ? (
          <>{t('shell.progressbutton.failed')}</>
        ) : (
          idleContent
        )}
      </span>
