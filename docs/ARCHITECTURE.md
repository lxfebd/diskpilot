...
DiskPilot 干三件事：**画一张磁盘地图 → 让你拖文件夹问 AI → 给少数几个常见软件配齐安全清理脚本**。三件事都跑在你本机，文件内容**绝不**离开你的电脑。
...
DiskPilot 在 Windows 上换了个法子：**直接读图书馆的"目录卡片柜"**——这柜子叫 **NTFS Master File Table（主文件表，简称 MFT）**，里面早就记好了"哪本书放哪、多大、谁的"。一次性把整个柜子扫完就够了，不用一架架翻。整盘 C: 通常 **2–5 秒**。

> 没有 MFT 的系统（macOS / Linux / 移动盘 / 网盘）会自动 fallback 到"一架架翻"模式（基于一个叫 `jwalk` 的高性能并行库），比 MFT 慢但仍然比传统工具快。

扫完之后，DiskPilot 把每个文件夹的大小算出来，画成两张图：
...
DiskPilot 的中间是一个聊天框。**把不认识的文件夹拖进去**，AI 用人话告诉你：
...
2. **每次操作记到 `~/.diskpilot/undo.jsonl`**——一个文本日志（路径在你电脑用户目录下），能查到"我啥时候删了什么"
3. **可选 7 天隔离区**：把删的东西放到 `~/.diskpilot/quarantine/` 暂存 7 天再清。给特别敏感的删除一道额外保险
...
└────────────┬───────────────┘
             ...
             ↓
┌────────────────────────────────────────────┐
│   Rust 后端 · 7 个独立 crate + Tauri 壳     │
├────────────────────────────────────────────┤
│  scanner       扫硬盘画地图                  │
│  scaffold      加载清理脚本                  │
│  executor      执行删除（回收站/隔离/直删）    │
│  scaffold-lint CI 校验工具                  │
│  steam-insp.   只读盘点 Steam 游戏库         │
│  toolbelt      图吧工具箱集成                │
│  agent-server  AI 可操作工具集（75 个，64+11）│
└────────────────────────────────────────────┘
```
...
- **steam-inspector** 只读解析 Steam 的 appmanifest / libraryfolders，不写盘
- **toolbelt** 集成"图吧工具箱"的 CLI 工具目录，文档即参数表
- **agent-server** 提供 75 个 MCP 工具给 AI（64 只读：磁盘/进程/硬件/网络/审计/回收站/环境变量/信息补全等，只出建议不删文件；11 写操作需确认门放行）

> AI 顾问能力不再有独立 crate：前端 `advisor/`（provider 路由）+ 后端 `ai.rs` 承担，`crates/advisor` 已删除。
...
这些是 DiskPilot **架构上保证的**，不是写在隐私政策里的"我们尽力"：
...
- [luolangaga/tubatools](https://github.com/luolangaga/tubatools)（GPL-3.0，3413★）—— 图吧工具箱 winUI3 版，工具目录/清单格式的权威参照

### 工具清单格式调研（2026-09）

对上游 `TubaWinUi3.WinUI3/Metadata/tools.json`（125 工具、14 个字段）做了字段统计，
DiskPilot 的 Tool Manifest 吸收其中的 **publisher / tags / tutorialUrl / downloadUrl** 四个字段
（语义对齐、URL 用各工具官方站而非图吧教程站）：

| 上游字段 | 出现次数 | DiskPilot 对应 |
|---|---|---|
| `match` | 125/125 | manifest `name`（工具名/目录名） |
| `description` | 89 | manifest `purpose` |
| `publisher` | 89 | manifest `publisher`（2026-09 新增） |
| `tags` | 89 | manifest `tags`（2026-09 新增） |
| `category` / `categories` | 21/55 | manifest `category`（跨分类入口走 `linked_from`） |
| `tutorialUrl` | 21 | manifest `tutorial_url`（2026-09 新增，官方站） |
| `downloadUrl` | 86 | manifest `download_hint`（2026-09 新增） |
| `archVariants` | 15 | catalog `arch` / `find_arch_variants` |
| `launchTarget` | 2 | catalog `launch_target` |

其余为上游特有（`order`/`builtin`/`wingetId`/`downloadFilter`），DiskPilot 不吸收：
分类顺序由 `category_order` 常量维护，内置操作（builtin）由前端识别而非数据驱动，
包管理安装暂不属于图吧工具箱的离线分发模型。
