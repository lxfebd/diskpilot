...
### 7.7 实测附录：QQ（9.x 经典版 + QQNT 双布局）

**勘测发现（2026-09-08，Windows 开发机）**：

- `%APPDATA%/Tencent/` 下存在 `QQ`（经典版数据根，实测只剩 `STemp`）、`QQNT`（**空目录**，装过但未登录）、`QQMusic`/`QQLive`/`QQEX`/`qqgameshare` 等**非目标**目录——`name_contains=["QQ"]` 会误伤它们，所以 QQ 的 `[match]` 不给 name_contains，只靠 detect 具体路径 + `**/Tencent/QQ` 通配。
- `%LOCALAPPDATA%/Tencent/` 只有 `Wemeet`，**QQ 不在 Local**——QQ 数据几乎全在 Roaming。
- 本机 QQNT 未登录 → 无法实测账号级目录树（`QQNT/users/<QQ号>/nt_qq/nt_data/...`）。`**/nt_data/**` 已在 scaffold-lint 红线样本（`QQ/nt_data/QQ/9.x/...`），**整个 nt_data 视为 L3**，任何 scope glob 不得命中。

**对 §2 分级的 QQ 映射**：

| scope id | label | glob | mode | prompt |
|---------|------|------|-----|--------|
| `temp-cache` | 临时文件（STemp/Temp） | `%APPDATA%/Tencent/QQ/{STemp,Temp}/**` | recycle | none |
| `received-files` | 接收的文件 | `**/Tencent/QQ/*/{FileReceive,FileRecv}/**` | recycle | days=30 |
| `received-images` | 接收的图片 | `**/Tencent/QQ/*/ImageRecv/**` | recycle | days=30 |
| `received-videos` | 接收的视频 | `**/Tencent/QQ/*/VideoRecv/**` | recycle | days=7 |
| `received-stickers` | 接收的表情 | `**/Tencent/QQ/*/CustomFaceRecv/**` | recycle | days=30 |

**保守决策（可追溯）**：

1. **QQNT 不覆盖**：nt_data 是红线大仓库（聊天 DB + config + QtWebView cookie），且无可信的目录树证据区分「可清缓存」与「红线」——宁可 0 命中不可误删。detect 保留 `%APPDATA%/Tencent/QQNT` 让用户看到卡片，scope 0 命中时显示「无可清理项」。
2. **账号目录用 `*` 单段**：`Tencent/QQ/*/ImageRecv/**` 只匹配 QQ 号目录（数字），不会命中 `Tencent/QQ/Accounts/`、`Tencent/QQ/nt_data/`、`Tencent/QQ/config` 等红线段（缺中间段或目录名不在括号内）。
3. **语音不单开**：QQ 的语音接收目录名（`Record`/`Audio`/`Voice` 变体）无可靠实测证据，不猜。微信的 `voice-attachments` 有明确 globe 依赖 `msg/attach`，QQ 没有同等证据。
4. **Brace 变体**（`FileReceive,FileRecv`）兼容 QQ 各版本把「接收文件」叫法不同的情况；`Image`（无 Recv）不并，因为部分版本 `Image/` 可能是头像缓存（L1）而非接收图片（L2），混放会模糊分级——头像缓存没有 QQNT 之外的可信路径，放弃。

**红线断言（写进 qq_pc_safety.rs）**：`QQ/nt_data`（QQNT 聊天/config/cookie）、`QQ/Accounts`、`Messageres*`（消息记录）、`*.db`、`config/`、`login/`、`key/`、`Favorite*` 全部 zero match。

### 7.8 实测附录：Discord（Electron 缓存布局）

**勘测发现（2026-09-08，Windows 开发机）**：

- `%APPDATA%/` 与 `%LOCALAPPDATA%/` 下均无 discord 目录（本机未安装）。Discord 数据布局依据为 Electron 应用通用结构 + 多个开源清理项目（同款 glob 已在广泛使用）。
- Discord 是纯 Electron：**全部数据在 `%APPDATA%/discord/`**（Roaming），`%LOCALAPPDATA%/discord` 只放更新器/安装残留（`Update.exe` 所在目录），无用户数据。缓存大头集中在 Roaming 的 Chromium 缓存区。
- 多账号：Discord 单实例多账号，登录态在 `Local Storage/`（红线），缓存区不区分账号——无需账号通配段。

**对 §2 分级的 Discord 映射**：

| scope id | label | glob | mode | prompt |
|---------|------|------|-----|--------|
| `cache` | Chromium 缓存 | `%APPDATA%/discord/Cache/**` | recycle | none |
| `code-cache` | 代码缓存（JS 编译） | `%APPDATA%/discord/Code Cache/**` | recycle | none |
| `gpu-cache` | GPU/WebGPU 缓存 | `%APPDATA%/discord/{GPUCache,DawnGraphiteCache,DawnWebGPUCache}/**` | recycle | none |
| `blob-storage` | Blob 临时存储 | `%APPDATA%/discord/blob_storage/**` | recycle | none |
| `crashpad` | 崩溃报告 | `%APPDATA%/discord/Crashpad/**` | recycle | none |

**保守决策（可追溯）**：

1. **红线区明确列全**：`Local Storage/`、`IndexedDB/`、`Network/`、`Session Storage/`（登录态 + 数据缓存，删了要重新登录/丢设置）、`Preferences`/`Cookies`/`Login Data`/`Local State`（顶层文件，Chromium 登录凭据）。`Cache` 之外的目录不加后缀白名单，靠目录名精确匹配 + 红线断言钉死。
2. **不开「图片缓存」独立 scope**：Discord 的媒体缓存混在 `Cache/`（f_xxxxx 无扩展名文件）里，无法按后缀分流，只能整目录清——这正是「全量清缓存」的典型场景（重新加载即重下）。
3. **不做 `%LOCALAPPDATA%` 探测**：`%LOCALAPPDATA%/discord` 只有更新器二进制，无用户数据；清它无收益且有损坏安装风险。
4. **版本目录兼容**：Discord 的 `app-<version>/` 子目录由更新器管理，不碰。数据目录名固定为 `discord`（大小写不敏感匹配），无需通配。

**红线断言（写进 discord_safety.rs）**：`Local Storage`、`IndexedDB`、`Network`、`Session Storage`、`Preferences`、`Cookies`、`Login Data`、`Local State` 全部 zero match。
