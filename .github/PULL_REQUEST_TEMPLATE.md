...
新增/重写 scaffold 请按 [development.md](../development.md) 的 14-phase 工作流走全流程，逐项确认：
...
- [ ] **Phase 9**: `cargo run -p diskpilot-scaffold-lint -- scaffolds/<id>.toml` 通过
- [ ] **Phase 10**: `crates/scaffold/tests/<id>_safety.rs` 存在；`cargo test -p diskpilot-scaffold` 通过；含**红线断言**
- [ ] **Phase 11**: 若改了 `Scaffold`/`Scope` 等结构，`cargo check -p diskpilot-desktop` + `pnpm -C apps/desktop exec tsc --noEmit` 都干净
- [ ] **Phase 12-13**: 若改 UI，套了 `<ErrorBoundary>`、用两步确认（不用 `window.confirm`）；`pnpm tauri dev` 实机跑过
- [ ] **Phase 14**: `docs/scaffold-requirements/<category>.md` 已更新（含实测路径映射）
